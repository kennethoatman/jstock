...
public class StockDatabaseJDialog extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jTable2.setModel(getStockInfoTableModel());
        ...
        this.jTable2.setDefaultRenderer(String.class, new StockTableCellRenderer());
        this.jTable2.setDefaultRenderer(StockInfo.Board.class, new StockTableCellRenderer());
        this.jTable2.setDefaultRenderer(StockInfo.Industry.class, new StockTableCellRenderer());
        this.jTable2.setDefaultEditor(Code.class, this.getCellEditor(Code.class));
        this.jTable2.setDefaultEditor(String.class, this.getCellEditor(String.class));
        this.jTable2.setDefaultEditor(StockInfo.Board.class, this.getCellEditor(StockInfo.Board.class));
        this.jTable2.setDefaultEditor(StockInfo.Industry.class, this.getCellEditor(StockInfo.Industry.class));
...
//        }
        this.result = new StockInfoDatabase(stockInfos);
    ...
    private void selectTable(int modelIndex) {
        clearAllTablesSelection();
    ...
    private void addNewStockInfo() {
        ...
        selectTable(selectedModelIndex);
    }
        ...
        return new Observer<AjaxAutoCompleteJComboBox, ResultType>() {
            ...
            public void update(AjaxAutoCompleteJComboBox subject, ResultType arg) {
                ...
                if (indexEx == null) {
                    ...
                    indexEx = getIndexEx(result.name, String.class);
                }
                if (indexEx != null) {
                    selectTable(indexEx.index);
                    // Warn the user.
    ...
    private TableModel getStockInfoTableModel() {
        return new StockInfoTableModel();
    }
...
//                    final String name = model.getStockInfos().get(entry.getIdentifier()).getName();
//                    List<String> pinyins = org.yccheok.jstock.gui.Utils.toHanyuPinyin(name);
...
//    }
    private class StockInfoTableModel extends AbstractTableModel {
        ...
        // CK - don't like this - we should have the StockInfoDatabase and edit it directly.
        // CK - but it's a big change and not that important.
        ...
        public void removeRow(int index) {
            if (!stockInfos.get(index).getUserDefined()) {
                return;
            }
        ...
        public StockInfoTableModel() {
            stockInfos = StockDatabaseJDialog.this.stockInfoDatabase.getStockInfos();
        }
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            switch (columnIndex) {
                ...
                case 1:
                    return stockInfos.get(rowIndex).getName();
                case 2:
                    return stockInfos.get(rowIndex).getBoard();
                case 3:
                    return stockInfos.get(rowIndex).getIndustry();
            }
        ...
        public void setValueAt(Object value, int row, int col) {
            if (col > 0 || stockInfos.get(row).getUserDefined()) {
                // easy...just edit it
                final StockInfo old = stockInfos.remove(row);
                switch (col) {
                    case 0:
                        final Code newCode=Code.newInstance(value.toString());
                        if (newCode!=old.code) {
                            stockInfos.add(row, StockInfo.newInstance(newCode, old.getName(), old.getIndustry(), old.getBoard(), true));
                        } else {
                            stockInfos.add(row,old);
                        }
                        break;
                    case 1:
                        if(!value.toString().equals(old.getName())) {
                            stockInfos.add(row, StockInfo.newInstance(old.code, value.toString(), old.getIndustry(), old.getBoard(), true));
                        } else {
                            stockInfos.add(row,old);
                        }
                        break;
                    case 2:
                        final StockInfo.Industry industry=StockInfo.Industry.valueOf(value.toString());
                        if (industry!=old.getIndustry()) {
                            stockInfos.add(row, StockInfo.newInstance(old.code, old.getName(), industry, old.getBoard(), true));
                        } else {
                            stockInfos.add(row,old);
                        }
                        break;
                    case 3:
                        final StockInfo.Board board=StockInfo.Board.valueOf(value.toString());
                        if (board!=old.getBoard()) {
                            stockInfos.add(row, StockInfo.newInstance(old.code, old.getName(), old.getIndustry(), board, true));
                        } else {
                            stockInfos.add(row,old);
                        }
                        break;
                ...
                // ought really to add a new one so the non-user one is still in the table
            }
        ...
        public int findStockInfoBySymbol(String string) {
            ...
            for (int i = 0, size = stockInfos.size(); i < size; i++) {
                if (stockInfos.get(i).getName().equalsIgnoreCase(string)) {
                    index = i;
                    break;
                }
            }
        ...
        public int addNewStockInfo() {
            ...
            this.stockInfos.add(StockInfo.newInstance(code));
        ...
        public int addNewStockInfo(ResultType result) {
            ...
            // Name from Yahoo means Symbol in JStock.
            this.stockInfos.add(StockInfo.newInstance(code, result.name, StockInfo.Industry.Unknown, StockInfo.Board.Unknown, true));
        ...
        private final String[] columnNames = {"Code", "Name", "Board", "Industry"};
        private final Class[] columnClasses = {Code.class, String.class, StockInfo.Board.class, StockInfo.Industry.class};
    }
    ...
    private class MyTableCellEditor extends DefaultCellEditor {
        ...
        public Object getCellEditorValue() {
            ...
            try {
                if (c == Code.class) {
                    return Code.newInstance(str.toUpperCase());
                } else if (c == String.class) {
                    return str;
                } else if (c == StockInfo.Board.class) {
                    return StockInfo.Board.valueOf(str);
                } else if (c == StockInfo.Industry.class) {
                    return StockInfo.Industry.valueOf(str);
                } else {
                    return null;
                }
            } catch (Exception ex) {
        ...
        public boolean stopCellEditing() {
            ...
            if (c == Code.class) {
                ...
                return super.stopCellEditing();  // anything goes
            } else if (c == StockInfo.Board.class) {
                try {
                    StockInfo.Board.valueOf(((String)(super.getCellEditorValue())).trim());
                    return super.stopCellEditing();
                }
                catch(Exception e)
                {
                ...
                try {
                    StockInfo.Industry.valueOf(((String)(super.getCellEditorValue())).trim());
                    return super.stopCellEditing();
                }
                catch(Exception e)
                {
    ...
    /**
     * Returns first encountered row index based on the string.
     * <code>null</code> if not found.
     *
     * @param string The string
     * @param c Code or Symbol class
     * @return first encountered row index based on the string.
     * <code>null</code> if not found
     */
