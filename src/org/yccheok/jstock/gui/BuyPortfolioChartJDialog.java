    ...
    private JFreeChart createChart(String name) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if(name.equals(cNames[0])) {
                ...
                if (isPenceToPoundConversionEnabled == false) {
                    dataExs.add(DataEx.newInstance(code.toString(), portfolioTreeTableModel.getCurrentValue(transactionSummary)));                
                } else {
                    dataExs.add(DataEx.newInstance(code.toString(), portfolioTreeTableModel.getCurrentValue(transactionSummary) / 100.0));
                }
