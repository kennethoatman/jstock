    ...
    public boolean openAsStatements(Statements statements, File file) {
        ...
        if (statements.getType() == Statement.Type.RealtimeInfo) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (codeStr.length() > 0) {
                    final Stock stock = Utils.getEmptyStock(Code.newInstance(codeStr));
...
//                }
            ...
            final Runnable r = new Runnable() {
                ...
                public void run() {
                    ...
                    if (((OperatorIndicator) indicator).getName().equalsIgnoreCase("fallbelow")) {
                        ...
                        message = MessageFormat.format(template, stock.code, lastPrice, price);
                    } else {
                    ...
                    if (jStockOptions.isPopupMessage()) {
                        displayPopupMessage(stock.code.toString(), message);
        ...
        if (this.jStockOptions.isSendEmail()) {
            final Runnable r = new Runnable() {
                ...
                public void run() {
                    ...
                    if (((OperatorIndicator) indicator).getName().equalsIgnoreCase("fallbelow")) {
                        final String template = GUIBundle.getString("MainFrame_FallBelow_template");
                        title = MessageFormat.format(template, stock.code, lastPrice, price);
                    } else {
                        final String template = GUIBundle.getString("MainFrame_RiseAbove_template");
                        title = MessageFormat.format(template, stock.code, lastPrice, price);
                    }
    ...
    public void displayHistoryChart(Code code) {
        final StockHistoryServer stockHistoryServer = stockHistoryMonitor.getStockHistoryServer(code);

        if (stockHistoryServer == null) {
            if (stockCodeHistoryGUI.add(code) && stockHistoryMonitor.addStockCode(code)) {
                final String template = GUIBundle.getString("MainFrame_LookingForHistory_template");
                final String message = MessageFormat.format(template, code.toString(), stockCodeHistoryGUI.size());
                setStatusBar(true, message);
            }
        } else {
            ChartJDialog chartJDialog = new ChartJDialog(MainFrame.this, code.toString(), false, stockHistoryServer);
        ...
        menuItem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                for (int row : rows) {
                    final int modelIndex = jTable1.getRowSorter().convertRowIndexToModel(row);
                    Stock stock = tableModel.getStock(modelIndex);
                    displayHistoryChart(stock.code);
                }
            }
        });
        ...
        if (jTable1.getSelectedRowCount() == 1) {
            ...
            menuItem.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent evt) {
                    ...
                    portfolioManagementJPanel.showNewBuyTransactionJDialog(stock.code, stock.getLastPrice(), false);
                }
            });
    ...
    private boolean saveStockInfoDatabaseAsCSV(Country country, StockInfoDatabase stockInfoDatabase) {
        ...
        final Statements statements = Statements.newInstanceFromStockInfoDatabase(stockInfoDatabase, false, true);
    ...
    private boolean saveUserDefinedDatabaseAsCSV(Country country, StockInfoDatabase stockInfoDatabase) {
        ...
        final Statements statements = Statements.newInstanceFromStockInfoDatabase(stockInfoDatabase, true, false);
    ...
    private StockInfoDatabase loadStockInfoDatabaseFromCSV(Country country) {
        File stockInfoDatabaseCSVFile = new File(org.yccheok.jstock.gui.Utils.getUserDataDirectory() + country + File.separator + "database" + File.separator + "stock-info-database.csv");
        ...
        java.util.List<StockInfo> stocks = new ArrayList<StockInfo>();
        for (int i = 0, ei = statements.size(); i < ei; i++) {
            ...
            StockInfo stock = StockInfo.newInstance(code, name, industry, board, false);
            stocks.add(stock);
        }

        stockInfoDatabaseCSVFile = new File(org.yccheok.jstock.gui.Utils.getUserDataDirectory() + country + File.separator + "database" + File.separator + "user-defined-database.csv");

        statements = Statements.newInstanceFromCSVFile(stockInfoDatabaseCSVFile);
        if (statements.getType() != Statement.Type.StockInfoDatabase) {
        ...
        for (int i = 0, ei = statements.size(); i < ei; i++) {
            ...
            StockInfo stock = StockInfo.newInstance(code, name, industry, board, true);
            stocks.add(stock);
        }
        ...
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                if (stock_info_database != null) {
                    ...
                    name = stock_info_database.codeToName(code);
                }
                ...
                if (stockCodeHistoryGUI.isEmpty()) {
                    if (runnable.getStockHistoryServer() != null) {
                        ...
                        final String message = MessageFormat.format(template, (name != null ? name : code));
                    ...
                    if (runnable.getStockHistoryServer() != null) {
                        ...
                        final String message = MessageFormat.format(template, (name != null ? name : code), stockCodeHistoryGUI.size());
                        setStatusBar(true, message);
                    } else {
                ...
                if ((runnable.getStockHistoryServer() != null) && shouldShowGUI) {
                    ChartJDialog chartJDialog = new ChartJDialog(MainFrame.this, (name != null ? name : code) + " (" + code + ")", false, runnable.getStockHistoryServer());
                    chartJDialog.setVisible(true);
                }
            }
        });
        ...
        return new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() >= 2) {
                    ...
                    if (stock_info_database != null) {
                        // Possible null if we are trying to get index history.
                        name = stock_info_database.codeToName(stock.code);
                    }
                    final String template = GUIBundle.getString("MainFrame_IntradayMovementTemplate");
                    final String message = MessageFormat.format(template, name == null ? stock.code : name);
